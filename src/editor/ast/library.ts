...
import { bossSpellLibrary } from '../../game/boss/spellLibrary';
...
export function registerCoreLibrary(evaluator: Evaluator): void {
	...
	// Boss Spell Library Functions (sc:: namespace)
	registerBossFunctions(evaluator);
}
...
function registerBossFunctions(evaluator: Evaluator): void {
	// sc::getBossColor() - Get Boss current color (red/blue)
	evaluator.registerNativeFunction('sc::getBossColor', [], () => {
		return bossSpellLibrary['sc::getBossColor']();
	});

	// sc::getBossPhase() - Get Boss current phase (0, 1, 2)
	evaluator.registerNativeFunction('sc::getBossPhase', [], () => {
		return bossSpellLibrary['sc::getBossPhase']();
	});

	// sc::getShieldCount() - Get Boss shield count (0-5)
	evaluator.registerNativeFunction('sc::getShieldCount', [], () => {
		return bossSpellLibrary['sc::getShieldCount']();
	});

	// sc::getBossPositions() - Get all Boss positions (for phantom phase)
	evaluator.registerNativeFunction('sc::getBossPositions', [], () => {
		// Return as Value array (array of any objects is valid Value[])
		return bossSpellLibrary['sc::getBossPositions']() as unknown as Value;
	});

	// sc::fireBlast(x, y) - Launch fire elemental attack
	evaluator.registerNativeFunction('sc::fireBlast', ['x', 'y'], (x, y) => {
		if (typeof x !== 'number' || typeof y !== 'number') {
			throw new Error(`sc::fireBlast requires two numbers (x, y), got ${typeof x} and ${typeof y}`);
		}
		return bossSpellLibrary['sc::fireBlast'](x as number, y as number);
	});

	// sc::iceBlast(x, y) - Launch ice elemental attack
	evaluator.registerNativeFunction('sc::iceBlast', ['x', 'y'], (x, y) => {
		if (typeof x !== 'number' || typeof y !== 'number') {
			throw new Error(`sc::iceBlast requires two numbers (x, y), got ${typeof x} and ${typeof y}`);
		}
		return bossSpellLibrary['sc::iceBlast'](x as number, y as number);
	});

	// sc::shootAtShield(index) - Attack specific shield (Phase 2)
	evaluator.registerNativeFunction('sc::shootAtShield', ['index'], (index) => {
		if (typeof index !== 'number') {
			throw new Error(`sc::shootAtShield requires a number (index), got ${typeof index}`);
		}
		return bossSpellLibrary['sc::shootAtShield'](index as number);
	});

	// sc::attackPosition(x, y) - Attack specific position (Phase 3)
	evaluator.registerNativeFunction('sc::attackPosition', ['x', 'y'], (x, y) => {
		if (typeof x !== 'number' || typeof y !== 'number') {
			throw new Error(`sc::attackPosition requires two numbers (x, y), got ${typeof x} and ${typeof y}`);
		}
		return bossSpellLibrary['sc::attackPosition'](x as number, y as number);
	});

	// sc::getDistanceToBoss() - Get distance from player to Boss
	evaluator.registerNativeFunction('sc::getDistanceToBoss', [], () => {
		return bossSpellLibrary['sc::getDistanceToBoss']();
	});
}
...
export const CORE_LIBRARY_FUNCTIONS = [
	...
	'tap', 'print',
	// Boss Spell Library (sc:: namespace)
	'sc::getBossColor', 'sc::getBossPhase', 'sc::getShieldCount', 'sc::getBossPositions',
	'sc::fireBlast', 'sc::iceBlast', 'sc::shootAtShield', 'sc::attackPosition', 'sc::getDistanceToBoss'
] as const;
